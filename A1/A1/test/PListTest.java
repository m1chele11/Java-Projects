...
class PListTest {
    ...
    @Test
    @DisplayName("empty: This test was self developed.")
    void emptytwo() {
        assertTimeoutPreemptively(Duration.ofSeconds(3), () -> {
            assertTrue(list.isEmpty());
            assertEquals(0, list.size());
    ...
    @Test
    @DisplayName("contains: This test was self developed")
    void containsTwo() {
        assertTimeoutPreemptively(Duration.ofSeconds(3), () -> {
            assertFalse(list.contains(10));
            assertFalse(list.contains(20));
            list = makeList(10,11,12,13,14);
            assertTrue(list.contains(10));
            assertTrue(list.contains(12));
            assertFalse(list.contains(20));
            assertFalse(list.contains(9));
            assertFalse(list.contains(15));
            list = makeList(10, 20,21,19);
            assertTrue(list.contains(10));
            assertTrue(list.contains(20));
    ...
    @DisplayName(" GET: index at 0")
    @Test
    public void testGetAtIndexZero() throws IndexOutOfBoundsException {
        // Initialize the list with elements
        list = makeList(10, 20, 30);

        // Check that get(0) returns the first element
        assertEquals(10, list.get(0));
    }
    @Test
    @DisplayName("GET: Out of Range Test")
    public void testGetAtIndexOutOfRange() {
        // Initialize the list with elements
        list = makeList(10, 20, 30);

        // Try to access an out-of-range index, should throw IndexOutOfBoundsException
